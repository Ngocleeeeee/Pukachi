...
namespace Assets.Script.Gravity
{
    public class CellAction : MonoBehaviour
    {
        ...
        {
            int point = 0;
            for (int column = 1; column < BaseGravity.MATRIX.GetLength(1) - 1; column++)
            {
                for (int row = 1; row < BaseGravity.MATRIX.GetLength(0) - 1; row++)
                {
                    if (BaseGravity.MATRIX[row, column] == 0)
                    {
                        point++;
        ...
        public virtual List<Vector3> GetListPosition(List<Cell> path)
        {
            ...
            Sprite firstSprite = BaseGravity.lstSprites[0];
