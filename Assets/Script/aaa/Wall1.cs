...
namespace Assets.Script.WallMode
{
    public class Wall1
    {
        ...
        private static List<Cell1> lstBrick;

        public Wall1(List<int> rows, List<int> cols)
        {
            Rows = rows;
            Cols = cols;
        }
        ...
        private List<Cell1> GetListCell()
        {
            
            lstBrick = new List<Cell1>();
            for (int j = 0; j < Base1.n+2; j++)
            {
                foreach (var row in Rows)
                {
                    lstBrick.Add(new Cell1(row, j));
                }

            }

            for (int i = 0; i < Base1.m + 2; i++)
            {
                foreach (var col in Cols)
                {
                    var newCell = new Cell1(i, col);
        ...
        private void DrawWall()
        {
            ...
            {
                GameObject brick;
                brick = GameObject.Instantiate(wallOBJ, Base1.gridParent);
