...
public class CellAction1 : MonoBehaviour
{
    private SpriteRenderer selectedImage; // Biến để lưu trữ ảnh đã chọn
    public int directionChanges = 0;
    Dictionary<Cell1, Cell1> PARENT;
    Cell1 CELL_FINAL, CELL_START;
    ...
    void Start()
    {
        ...
        lineObject.transform.SetParent(Base1.gridParent);
    ...
    void CheckAndProcessMatrix(GameObject obj1, GameObject obj2)
    {
        ...
        var cell1 = Base1.GetCell(obj1.name);
        var cell2 = Base1.GetCell(obj2.name);
        if (cell1 == null || cell2 == null) return;
        // Kiểm tra nếu cả hai đối tượng có cùng số trong ma trận và không phải là cùng một đối tượng
        if (Base1.MATRIX[cell1.i, cell1.j] == Base1.MATRIX[cell2.i, cell2.j] && obj1 != obj2)
        {
            ...
            if (path != null)
            {
                ...
                Destroy(obj1);
                Destroy(obj2);
                var val1 = Base1.MATRIX[cell1.i, cell1.j];

                Base1.FREQUENCY[val1] -= 2;
                if (!Base1.FREQUENCY.ContainsKey(0))
                {
                    Base1.FREQUENCY[0] = 0;
                }
                Base1.FREQUENCY[0] += 2;

                Base1.MATRIX[cell1.i, cell1.j] = 0;
                Base1.MATRIX[cell2.i, cell2.j] = 0;
    ...
    List<Cell1> FindPath(Cell1 cellStart, Cell1 cellFinal)
    {
        if (!Base1.IsCellInMaxtrix(cellStart) || !Base1.IsCellInMaxtrix(cellFinal)) return null;

        // Khởi tạo hàng đợi để duyệt các đỉnh theo BFS
        Queue<Cell1> queue = new Queue<Cell1>();
        queue.Enqueue(cellStart);
       
        // Khởi tạo từ điển để lưu trữ các đỉnh trước đó
        Dictionary<Cell1, Cell1> parent = new Dictionary<Cell1, Cell1>();
        ...
        while (queue.Count > 0)
        {
            Cell1 cellCurrent = queue.Dequeue();
        ...
        PARENT = new Dictionary<Cell1, Cell1>();
    ...
    private List<Cell1> DFS(Cell1 cellCurrent)
    {
    ...
    private List<Cell1> GetPath(Cell1 cellCurrent1, Cell1 cellStart1, Dictionary<Cell1, Cell1> parent)
    {

        List<Cell1> path = new List<Cell1>();
        var cellCurrent = new Cell1(cellCurrent1.i, cellCurrent1.j);
        var cellStart = new Cell1(cellStart1.i, cellStart1.j);
    ...
    private bool IsValidPath(List<Cell1> path1) 
    {
        List<Cell1> path = new List<Cell1>();
        path.AddRange(path1);
    ...
    private int GetChangeDirection(List<Cell1> path)
    {
        int cnt = 0;
        var baseCell = new Cell1(path[0].i, path[0].j);
        for (int i = 1; i < path.Count; i++)
        {
            var moveCell = new Cell1(path[i].i, path[i].j);
            ...
            //WALL MODE
            /*if (Wall1.IsBlockByBrick(path[i].i, path[i].j))
            {
                return 100;
            }*/
    ...
    void PrintPath(List<Cell1> path)
    {
        if (path == null)
        {
    ...
    private List<Vector3> GetListPosition(List<Cell1> path)
    {
        var res = new List<Vector3>();
        Sprite firstSprite = Base1.lstSprites[0];
